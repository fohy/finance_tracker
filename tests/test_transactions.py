...
def test_transfer_updates_both_balances_and_can_be_reversed(client, csrf_headers):
    ...
    response = client.post("/api/transactions", headers=csrf_headers, json={
...
def test_transaction_rejects_invalid_references_and_preserves_balances(client, csrf_headers):
    data = bootstrap(client)
    life = next(account for account in data["accounts"] if account["kind"] == "life")
